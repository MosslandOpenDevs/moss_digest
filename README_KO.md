...
v1은 **수집 → 요약 → 보고서 생성**의 전체 파이프라인을 다룹니다. 다음 단계는
MossDigest를 *일괄 보고서 생성기*에서 *점진적(incremental)이고 제공자에 독립적인
활동 인텔리전스 파이프라인*으로 발전시키는 것입니다. 아래 항목은 의도적으로
열어둔 방향들입니다 — 실 하나를 잡아 당기면 됩니다. 코드베이스는 작고
모듈화(`collectors` / `generators` / `utils` / `commands`)되어 있어 대부분
독립적인 모듈이나 명령어로 추가할 수 있습니다. 기여를 환영합니다.

### 성능 & 신뢰성
- **레이트리밋을 고려한 동시 수집.** 현재 조직 수집은 저장소별로
  (커밋·릴리즈·PR·이슈·기여자) 완전히 순차 처리됩니다. 제한된 동시성 또는 GitHub
  GraphQL API로 대규모 조직의 소요 시간을 단축.
- **증분 캐싱.** 저장소/기간별 스냅샷을 저장해 재실행 시 변하지 않은 이력을 다시
  가져오지 않도록 하고, 레이트리밋에 대해 부드럽게 백오프.
- **소스별 실패를 로그에만 남기지 말고 보고서에 노출** — 부분 수집이 조용히
  넘어가지 않고 드러나도록.

### 데이터 품질
- **기간 정확도가 있는 기여자 통계.** `listContributors`는 날짜 범위를 지원하지
  않으므로, 기간 내 커밋에서 기여자를 도출.
- **실제 커밋 라인 통계.** `listCommits`는 additions/deletions를 반환하지 않음 →
  `getCommit`/GraphQL로 보강하거나 항상 0인 필드를 제거.
- **외부 링크 중복 fetch 제거 + LLM 요약 캐싱** (현재 각 링크를 두 번 가져오고,
  요약을 매 실행마다 재계산).

### 요약(LLM)
- **제공자 독립 LLM 레이어.** LM Studio·Gemini·OpenAI 호환·Anthropic을 하나의
  인터페이스 뒤에 두고 우아한 폴백을 제공 — 제공자 전환이 코드 수정이 아니라 설정
  변경이 되도록.

### 산출물 & 배포
- **상세 PDF 보고서 재활성화** (Puppeteer 경로는 이미 존재).
- **더 풍부한 웹 엔트리 페이지:** 검색/필터, 기간 간·전년 대비(YoY) 트렌드,
  GitHub Pages로의 원커맨드 배포.
- **알림 연동.** `.env`에 이미 있는 `SLACK_WEBHOOK_URL` / `DISCORD_WEBHOOK_URL`이
  아직 미사용 — 완료 시 보고서 링크 전송.

### 개발 경험
- **테스트 확장**: Octokit/RSS를 목킹한 통합 테스트와 HTML 스냅샷 테스트, 그리고
  모든 PR에서 `npm test`를 돌리는 **GitHub Actions CI** 추가.
- 일관된 스타일을 위한 ESLint/Prettier 도입.
